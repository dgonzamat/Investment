...
from app.services.strategy_v7 import evaluate_signal_v7
...
class Backtest:
    ...
    def _step_v7(self, window, cash, shares, position_state):
        result = evaluate_signal_v7(window, position_state)
        self._last_v7_meta = {'last_check_idx': result.get('last_check_idx', len(window))}
        return (result['action'], result.get('position_size', 0),
                result.get('stop_price'), result.get('reason', ''))
    ...
    def run(self) -> dict:
        ...
        if self.strategy == 'v7':
            step_fn = self._step_v7
        elif self.strategy == 'v6':
        ...
        for i in range(self.warmup_days, len(self.ohlcv)):
            ...
            if action == 'BUY' and shares == 0:
                ...
                if cost <= cash and shares_to_buy > 0:
                    ...
                    # For v7: same monthly tracking
                    if self.strategy == 'v7' and hasattr(self, '_last_v7_meta'):
                        position_state['last_check_idx'] = self._last_v7_meta['last_check_idx']
                    trades.append({
